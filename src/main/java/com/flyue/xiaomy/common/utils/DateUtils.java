...
/**
 * @Author: Liu Yuefei
 * @Date: Created in 2019/6/25 17:32
 * @Description:
 */
...
public class DateUtils {
    ...
    public static final String DATE_HOUR_STR_1 = "yyyyMMdd_HH";
...
//    }
    ...
    public static long dateToUnixTimestamp(String date) {
        ...
        try {
            ...
            logger.error("将指定的日期转换成Unix时间戳失败", e);
            e.printStackTrace();
        }
        return timestamp;
    }
    ...
    /**
     * 根据数据库的时间获取找药易的时间格式 规则：规则： <1 min:刚刚；1min-60min:X分钟前；1h-24h:
     * X小时前；>24h:YYYY-dd-mm）
     *
    ...
    public static Date localDateToDate(LocalDate localDate) {
        Instant instant = localDate.atStartOfDay().atZone(DEFAULT_ZONEID).toInstant();
    ...
    public static LocalDate dateToLocalDate(Date date) {
        ...
        LocalDateTime localDateTime = LocalDateTime.ofInstant(instant, DEFAULT_ZONEID);
        return localDateTime.toLocalDate();
    }

    public static Date startOfDay(Date date) {
        LocalDateTime now = LocalDateTime.ofInstant(date.toInstant(), DEFAULT_ZONEID)
    ...
    public static void main(String[] args) {
        Date date = localDateToDate(LocalDate.now());
